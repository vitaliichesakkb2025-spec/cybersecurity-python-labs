"""Перевірка доступу з пріоритетом блокування над допуском."""
...
def check_access(username, resource, users=USERS, blocked=BLOCKED_USERS):
    """Перевірити існування, блокування, активність і рівень допуску."""
...
def run():
    ...
    for name, level in RESOURCES:
        print(f"{name:<28} {SECURITY_LEVELS[level - 1]}")
